...
class PARSeq_PD_1(CrossEntropySystem):
    """PARSeq with scheduled parallel decoding. 
    changed to add padding mask, and ignore pad_index in loss.
    """
    ...
    def training_step(self, batch, batch_idx) -> STEP_OUTPUT:
        ...
        tgt_padding_mask = (L_ids == self.pad_id)
        out, _ = self.decode(L_ids, memory, tgt_padding_mask=tgt_padding_mask, tgt_query_mask=attn_mask_OL, tgt_query=O_embs)
        logits = self.head(out)
        loss = F.cross_entropy(logits.moveaxis(-1, 1), L_ids_tgt, ignore_index=self.pad_id)
